...
float fs_mag_map[] = { 4, 8, 12, 16 };

LIS3MDL::LIS3MDL(void)
{
  _device = device_LIS3MDL;
...
/*
void LIS3MDL::setAddress(uint8_t addr)
{
  address = addr;	
}
*/
...
int16_t LIS3MDL::readY(void)
{
  Wire.beginTransmission(address);
  // assert MSB to enable subaddress updating
  Wire.write(OUT_Y_L | 0x80);
  Wire.endTransmission();
  Wire.requestFrom(address, (uint8_t)2);
...
int16_t LIS3MDL::readZ(void)
{
  Wire.beginTransmission(address);
  // assert MSB to enable subaddress updating
  Wire.write(OUT_Z_L | 0x80);
  Wire.endTransmission();
  Wire.requestFrom(address, (uint8_t)2);
...
void LIS3MDL::readXYZ(uint16_t val[3])
{
  Wire.beginTransmission(address);
  // assert MSB to enable subaddress updating
  Wire.write(OUT_X_L | 0x80);
...
float LIS3MDL::getScaledVal(int16_t raw)
{
	return (raw * fs_mag_map[0]) / 32768.0;
}
